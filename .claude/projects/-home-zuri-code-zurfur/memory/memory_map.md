...
- **backend/crates/api** — composition root; HTTP/config/tracing. No CLAUDE.md yet; config/runtime detail in memory `config-and-runtime`; governing DDs `23592962` (RFC 9457 error model), `24543244` (Auth Surfaces/CSRF) — add a checkpoint here if api-local questions recur.
...
Memory layer: repo-local memories = feedback (tests, no-generic-names, readability, assign-before-Ok), vision, config-and-runtime, this map; global = `project-zurfur`. Slug is `-home-zuri-code-zurfur` (fixed from a stale `-Documents-zurfur` on 2026-06-30).
