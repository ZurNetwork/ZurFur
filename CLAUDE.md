...
**All design lives in the design corpus — it is the single source of truth.** The corpus is the sibling repo `ZurNetwork/zurfur-design`, checked out at `~/code/zurfur-design` (`ZURFUR_DESIGN_DIR` overrides the path). One Markdown page per file, named `<id>-<slug>.md`, under `entities/` (the glossary: User, Account, Character, Commission, Golem, Plugin, …), `decisions/` (the DDs), `project/` (scope — "Project MVP" — philosophy, conventions), `data-layer/`, and `superseded/` (pages that govern nothing anymore). Work is tracked in the Jira project ZMVP. Do not create local design documents in this repo; read and update the corpus instead. Confluence DESIGN is a frozen read-only archive — it is neither read nor written.

**Read before guessing.** Many decisions live only in the corpus, not in code. When unsure whether something is already decided/defined and it sounds familiar, **read the relevant design page before asking or asserting from memory.** A generated pointer index of every page (id, status, title) is at @docs/design-index.md — match the topic there, then open that page in the corpus, or `rg` the corpus for the wording. Pages under `superseded/` are hidden from ripgrep by the corpus `.ignore`; open one by path when you need the history behind a live page. If the corpus isn't readable in this session, say so and stop — never fall back to Confluence or memory. Only ask once the page doesn't resolve it, or when a genuinely new decision is needed (route through `/design-decision`).

## Memory & references

- **References, not copies.** The corpus is the source of truth; memory and every `CLAUDE.md` hold **pointers** (page id + title), never copied page bodies. When in doubt, **read** the page (via the index above) rather than assert from memory.
...
- Tend this layer with `/optimize-memory`; file a single non-design reference with `/save-reference` (design pages need no pointer memory — they are in the corpus and in the generated index).
...
- **Design in sync** — if a documented entity/flow changed, the design corpus matches the Engineer's decisions (`/design-sync`); a big decision is captured as a DD (`/design-decision`).
...
Every directory that carries information beyond its parent has a `NODE.json` — *what/where*, while `CLAUDE.md` is *how to behave*. A node's `short` is the glance (what `just nodes tree` prints); `is` is the full sentence. The tree is read and written through the `nodes` tool (github.com/ZurNetwork/nodes, pinned by `NODES_VERSION` in the Justfile; `just setup` installs it): `just nodes tree` (the whole tree), `just nodes chain <path>` (root → … → node — what `/familiarize` loads for a path), `just nodes get <path> [field]`, `just nodes refs <pageId>` (every node citing a design page — `/design-sync` step 6), `just nodes find <term>`, and for writes `just nodes set <path> <field> '<json>'` · `just nodes add-ref <path> <page> "<title>" "<governs>"` (quote multi-word arguments; they pass through intact) · `rm-ref` · `touch`. A PostToolUse hook (`.claude/settings.json` → `scripts/nodes-fmt-hook.sh`) formats any `NODE.json` a model writes; `just nodes-check` (inside `just gate`, and CI's `nodes` job) validates the tree against the schema and `docs/design-index.md`. That index is generated from the corpus by `just design-index`, and `just design-index-check` (also inside `just gate`) reports when it has drifted from the corpus.

**Doc comments exist in a vacuum.** A `///`, `//!`, JSDoc, SQL-header or `.proto` comment states its fact plainly and never carries a DD number, design page id, Jira key, PR number or "per ruling" pointer — every code→design pointer lives in the nearest `NODE.json` under `refs`. Documentation too big for its file (module essays, migration rationale) moves to that node's `notes`, with a one-line summary left in place.

## Architecture

Ports and adapters, per the design page "Domains and Applications" (`11763713`):
...
- **Work happens in the primary checkout, on jj; git worktrees are retired** (ruling 2026-07-20). `/start` opens the ticket's change there, no worktree question. A genuinely parallel set gets a **jj workspace** under `~/code/zurfur-<slug>`, on the Engineer's go. Tests are isolated either way (each spins up its own testcontainers Postgres on a random port); the **dev stack is not** — a second checkout that runs `just dev` needs its own DB/HTTP ports and compose project name in its `.env` (see `.env.example`).
