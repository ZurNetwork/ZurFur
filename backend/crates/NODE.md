...
fs:
  - name: domain/
    ...
    node: true
  - name: application/
    role: use cases — one plain async fn per use case, called by every driver (DD 55836674)
    node: true
  - name: composition/
    role: the HTTP-free composition root shared by api + cli — Config + the live Runtime port bag
    node: true
  - name: shared/
    role: zero-dependency leaf crate of build-time constants domain may consume
    node: true
  - name: api/
    role: composition root's HTTP driver + axum surface + generated contract types
    node: true
  - name: cli/
    role: the zurfur terminal driving adapter — second driver over the same application use cases
    node: true
  - name: adapter-pg/
