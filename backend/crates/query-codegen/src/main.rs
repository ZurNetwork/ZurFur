...
/// own crate default (`11-alpine`) — the schema now uses PG12+-only features
/// (e.g. `GENERATED ALWAYS AS (...) STORED`, ZMVP-164), which the crate
/// default can't even parse. Mirrors `test-support::pg::POSTGRES_TAG`.
