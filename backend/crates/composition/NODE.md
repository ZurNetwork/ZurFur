...
**Conventions:** HTTP-free by construction, enforced by `tests/no_http_deps.rs` — never add axum/tower-sessions here or to `cli`. Migrations, background tasks, sessions and cookies are explicitly NOT this crate's concern — left to the driver. The Runtime→`application::*Ports` conversion lives once in `src/ports.rs` (one `From<&Runtime>` per application module); drivers never hand-assemble a Ports struct.
...
- Two ports are pool-backed on purpose, the documented exceptions to the unit of work: the `profile_cache` fill (a read-path write with no transactional invariant) and `files` (blob bytes cannot ride a transaction).
