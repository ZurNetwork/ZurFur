...
//! spelling — until the contract moves to a leaf crate that both drivers can
//! share; the same guard `create_account_parity.rs` gives `POST /accounts`.
...
fn account_delete_renders_exactly_like_delete_accounts() {
    ...
    // derives none of `Debug, Clone, PartialEq, Eq`, so it
