-- The `commission_invitation` table backing seat invite-then-accept. A row is
-- a pending offer of a commission Seat until it is accepted or revoked — the
-- Seat mirror of `account_invitations`. `state` is the InvitationState::as_str
...
-- account-invitation's issuer. Singular table name — the house convention
-- (the plural `account_invitations` is the old inconsistency).
--
...
-- resolved on the accept path, not by this table); only a duplicate pending for
