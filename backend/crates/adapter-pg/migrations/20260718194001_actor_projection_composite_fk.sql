-- Bind each projection table to the
...
-- merely checked. Runs after an earlier backfill, so every existing row already has its
-- (id, kind) parent.
--
...
-- DEFAULT. That default is deliberate and permanent (not a backfill-then-
...
-- Per-kind DID guarantee. Users and accounts ALWAYS carry a DID: their
