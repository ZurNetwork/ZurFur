-- The `account_invitations` table backing invite-then-accept. A row is a
-- pending offer of account membership until it is accepted or revoked.
-- `role` and `state` are the Role::as_str / InvitationState::as_str
-- discriminants; `inviter` is recorded because on acceptance they become the
-- new member's Parent.
--
