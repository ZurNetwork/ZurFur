...
- **Transactions are a compile-enforced capability** — the Unit of Work is enforced by the type system, not by convention (DECIDED, built in ZMVP-36): writes live only on the tx-bound `UnitOfWork` views (`uow.accounts()`/`uow.users()`); a bare-pool write (`.execute(&self.pool)`) is unrepresentable and the `tests/no_bare_pool_writes.rs` guard backs the residual adapter-internal hole. Three sites are **documented exemptions** (no transactional invariant): `session_store`, `auth_store` (adapter-atproto), `profile.rs` cache fill — a *new* exemption is a design question, not a quiet edit. → DD *Transactions as a capability — a compile-enforced Unit of Work in the private store* (`24150017`).
