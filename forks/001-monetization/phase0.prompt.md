...
# PHASE 0 — GROUNDING (this phase only)

List every empirical claim you intend to argue from (prices, fees, processor policies, laws, competitor take rates, grant deadlines, market facts) as a numbered list. For EACH claim, fetch and cite a source (URL + what it says) using your web search capability. Claims you cannot source, mark UNSOURCED — the judge will strike them and you may not argue from them later. Flag any claim in the brief's research snapshot you believe is now wrong or outdated, with a source. Do NOT argue a position yet. Output: numbered claims, each with source URL and a one-line quote/paraphrase.
