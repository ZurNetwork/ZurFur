#!/usr/bin/env python3
"""Boardroom runner for fork 003 (character PDS) — SCRUTINIZE mode. Seats: GPT + Gemini. Claude judges (odd fork).
...
PHASES = {
    "S0": (
        "PHASE S0 — VERIFICATION. For each enumerated claim 1–11 give exactly one verdict: CONFIRMED, "
        "WRONG (with source and the corrected fact), or UNVERIFIABLE. Repo-internal claims: verify against "
        "the context block only. External claims (1, 2, 3, 4, 5, 7, 9): "
    ...
    "S1": (
        "PHASE S1 — ATTACK (isolated). For EACH option A–D give: (a) the strongest case AGAINST it, (b) the "
        "strongest case FOR it, (c) one concrete failure scenario (state/inputs → wrong outcome). Use every claim in its corrected form from the consolidated S0 verdicts. MANDATORY at the end: name at least one option or consideration "
        "MISSING from the brief (attack the framing), or state explicitly that the option space is complete "
        "and why. Label any new option E, F, … with a one-line definition."
    ),
    "S2": (
    ...
    "S3": (
        "PHASE S3 — INDEPENDENT RANKING. Rank the FULL option set — A–D plus every board-added option listed "
...
def main():
    ...
    def suffix(seat):
        s = PHASES[phase]
        if phase == "S1":
            # the judge's consolidated S0 verdicts — corrected claims replace the originals from here on
            s += "\n\n=== CONSOLIDATED S0 VERDICTS (judge) ===\n\n" + open(os.path.join(DIR, "S0-judge.md"), encoding="utf-8").read()
        if phase == "S2":
