...
| R10 | **Notice window: 12 months, bound to per-major telemetry** (resolves §8 Q7; ruled 2026-07-27). ≥ 12 months from `Deprecation` to `Sunset` for any GA path-major (Google Cloud ToS floor); pre-GA surfaces get zero. The commitment is honest only with eyes: Zalando #188 **per-major request-count telemetry is adopted alongside it** — a major retires when the numbers show migration, never calendar-blind. 12 is a floor, not a ceiling. |
...
| **Preview surfaces — all three** | **zero** | Graph: beta breaking changes "without notice"; Azure: "Between preview versions, a service can break any of the newly added preview features"; Google Cloud ToS: policy "does not apply to … pre-general availability Services, offerings, or functionality." |
...
**7.8 Route metadata.** `google/api/annotations.proto` declares `extend google.protobuf.MethodOptions { HttpRule http = 72295728; }` (https://raw.githubusercontent.com/googleapis/googleapis/master/google/api/annotations.proto) and is not gRPC-locked — it is a descriptor annotation. Three versioning consequences: (i) `additional_bindings` is the built-in way to serve an old and a new path from one method — the natural carrier for a **route-level** deprecation window, distinct from field-level deprecation; (ii) "All other fields are passed via the URL query parameters" means **adding a request field silently adds an accepted query parameter** — additive and safe, but it means the URL surface is derived from the message rather than declared, which this contract should state; (iii) `{var}` ≡ `{var=*}` matches a **single path segment**, so any id containing `/` (AT-URIs do) needs an explicit `{var=**}` template. Whether a Rust/axum codegen path actually consumes `HttpRule` is **UNVERIFIED** and is a build-side question, not a policy one.
...
- That `bufbuild/buf-action@v1` runs `breaking` cleanly with **no `BUF_TOKEN`** (registry-less setup). Test before wiring CI.
- That buf has **no** deprecation-window/sunset feature anywhere — negative-verified across the pages fetched, not an exhaustive crawl.
- ProtoJSON encoding of `google.protobuf.Struct`/`Value` for our `payload: serde_json::Value` fields — in particular whether numeric precision survives (`Value` numbers are doubles). Not researched; **this is a live int64-precision hazard inside the payload blob**, independent of §7.2.
- Whether any Rust/axum codegen path consumes `google.api.HttpRule`.
- Browser / proxy / CDN handling of `Deprecation` and `Sunset` in practice.