...
**Clause:** any response that carries the deprecation triple MUST also carry an `Access-Control-Expose-Headers` header whose value **includes** the tokens `Deprecation`, `Sunset`, and `Link` (inclusion, not exact equality — the exposed list may grow; emitted by the same middleware that stamps the triple, one derived path per §4 assertion 4). The first-party BFF is same-origin and unaffected; the public bearer-token surface (`/plugin/v1`) is the textbook cross-origin consumer and the reason this clause exists. Enforced by §4 assertion 7.
...
7. **The triple is CORS-visible.** Every response carrying `Deprecation`/`Sunset` also carries an `Access-Control-Expose-Headers` value that includes `Deprecation`, `Sunset`, and `Link` (§3's CORS clause); a test asserts the three tokens are present — inclusion, not exact equality — alongside assertion 4's byte checks.
...
**7.9 `google.protobuf.Struct`/`Value` cannot carry a large integer — the payload-blob hazard (NEW 2026-07-27, Q8 sweep).** `Value.number_value` is declared `double` (https://protobuf.dev/reference/protobuf/google.protobuf/, struct.proto); there is no int64 arm, so §7.2's decimal-string rescue **does not reach inside a `Struct`** — an integer above 2⁵³ in a payload blob is not at risk, it is unrepresentable. The tiers then diverge on failure mode: workspace-pinned `pbjson-types` 0.8.0 **errors** on an out-of-range integer (`visit_i64`: "out of range integral type conversion attempted" — while its `visit_f64` accepts `1e30` lossily, a guard inconsistent by magnitude, not lossiness), whereas protobuf-es receives values **already through `JSON.parse`** and silently truncates (MDN: "numbers in JSON text will have already been converted to JavaScript numbers, and may lose precision in the process"). Same bytes: Rust 500s, TypeScript stores wrong data quietly. **Not live today** — the corpus contains no `Struct`/`Value`, and `payload: serde_json::Value` exists only in the hand-written changelog handler — but it **arms the day the changelog message enters the corpus**, at which point stored payloads that round-trip today can start failing to deserialize. → **RESOLVED by R11** (typed per-kind for the changelog's closed vocabulary; opaque JSON string for plugin-owned payloads — ownership decides).

**7.10 R11's third case — mixed ownership over a deferred vocabulary (NEW 2026-08-08, ZMVP-163).** R11 decides representation by **ownership**: typed per-kind messages where Zurfur owns the vocabulary, an opaque JSON `string` where a plugin does. The commission's flat composition (DD [45514754](https://zurnetwork.atlassian.net/wiki/spaces/DESIGN/pages/45514754)) is neither, and is the case R11's two arms do not name: `CommissionElement.payload` carries **core-written and plugin-written content through the same field**, because there is no privileged core path — core is the first consumer of the same extension points (DD D2). Zurfur will own part of that vocabulary, and the type catalog that would say which part is deliberately not written yet (D10, ZMVP-171).

**The ruling: mixed ownership over a vocabulary that does not exist yet takes the opaque form now, with typed arms added later — and the field is declared a `oneof` at birth so that "later" stays additive.**

```proto
oneof payload {
  string opaque_json = 6;   // today, for every element type
  // NoteContent note = 7;  // tomorrow, per kind, additively
}
```

Three things make this a rule rather than a hedge:

- **The opaque string is the safe half of an unequal bet.** Serving typed content as text costs a `JSON.parse`; serving plugin-owned or not-yet-designed content as `Struct` costs §7.9's divergence — Rust 500s where TypeScript silently truncates. Choosing wrong in the opaque direction is recoverable, choosing wrong in the structured direction corrupts data on one tier only.
- **The `oneof` is the one-way door, and it is taken at the mint.** `FIELD_SAME_ONEOF` means a field shipped *outside* a `oneof` can never move into one, so a bare `string payload` would have foreclosed every typed arm forever. Wrapping it costs one line and nothing on the wire — a single-arm `oneof` emits identical bytes.
- **A deferred vocabulary is not the same as an unowned one.** R11's plugin arm is opaque *by mandate* (core is forbidden to know the structure). This case is opaque *by schedule*. The two must not be conflated: when the catalog lands, adding typed arms for the kinds Zurfur owns is expected, and the plugin-written kinds stay on `opaque_json` permanently.

The companion rule is R3's, and it belongs here because the same message is where it bites: **the envelope around an opaque payload must not carry a stored ordinal.** `CommissionElement` deliberately has no `position` and no `band` — a *sparse* ordinal on a projected list counts what the projection removed, which turns a visibility filter into a disclosure. Order is served, densely, after the filter.
...
**Q9 — How does an unstructured `payload` blob cross the wire?** *(NEW 2026-07-27, opened by the Q8 sweep; see §7.9.)* `google.protobuf.Struct` cannot carry an integer above 2⁵³, and the two tiers disagree on the failure mode — Rust rejects, TypeScript truncates. **Closed by** an Engineer ruling among: (a) declare payload as `Struct` and bind a contract rule that payload numbers fit ±2⁵³, enforced at write; (b) carry payload as an opaque JSON `string`/`bytes`, moving precision outside ProtoJSON entirely; (c) type the payload per changelog `kind` as real proto messages and retire the free-form blob. **Must be decided before the changelog message enters the corpus** — not after stored payloads start failing to deserialize. → **RESOLVED 2026-07-27 at the ratification read: (c) for the changelog, (b) for plugin-owned payloads — see R11.** The deciding principle is vocabulary ownership: the changelog's kinds are a closed, Zurfur-defined vocabulary (Eventlog DD 32178178 D1), so its payload shapes are declared in the contract as per-kind messages — Zurfur's own language, `buf breaking`-guarded, the 2⁵³ hazard unrepresentable. Plugin source-stream payloads are the opposite case — core is forbidden from knowing their structure — and cross any future wire as opaque JSON strings, precision untouched.