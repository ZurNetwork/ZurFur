...
- New DD pages should be created via `/design-decision` (writes the Confluence page + Jira tickets). Index snapshot built 2026-06-30; incrementally updated 2026-07-02 (added DD 27852802, relabeled 24870914) and 2026-07-04 (ZMVP-104: added the AT Protocol Boundary Contract programme — 29622283 / 29687820 / 29949954 / 29982722 / 29622362 / 29622321 / 29818896 — and the Replyable DD 30572573). **Full `getPagesInConfluenceSpace` re-list done 2026-07-11** (space held 102 pages; 35 added below in "2026-07 wave" sections) during the full-space `/design-sync` audit. Incremental 2026-07-22: added DD 39944194 (frontend stack). Incremental 2026-07-25: added DD 40992770 (the API contract), the new `Portal` entity page, and the four `Plugin` sub-pages created when Plugin was split; moved `Golem` out of the data-layer section into core entities where it belongs. Incremental 2026-07-28: added DD 42762241 (surface tree wire shape). Incremental 2026-08-08: added DD 47939586 (the Rust Frontend — Leptos) — **REVERTED 2026-08-09 by the Engineer; the frontend stays plain SvelteKit + TypeScript (39944194 governs), and ZesTTY/the Dialect (46596098) is also retired.** Incremental 2026-08-27: added DD 55836674 (the application layer) + the missing 54427650 pointer. Incremental 2026-08-30: added DD 57081857 (actor addressing — DID everywhere). Incremental 2026-08-04: added DD 45514754 (commission composition — flat extension points), superseding-in-part 28246028 / 28409880 / 42762241. Incremental 2026-09-04: amended 29130754 inline (view grants per-User) + 8 dependent pages; added DD 59310081 (the changelog as the only record; Eventlog = frontend Surface), superseding-in-part 32178178 / 30408741. Incremental 2026-09-11: added DD 62488577 (board ordering — fractional Position keys via LexOrdering, anchor placement, one card per board, invariants domain-owned); superseding-in-part 28409880 (D3's fractional-upgrade open row TAKEN for workflow boards).

## Settled-decision quick facts

Know these without fetching; fetch the linked DD page for detail.

- `did:plc` everywhere · ~~Character has its own DID day one~~ ~~superseded 2026-07-14: Characters are actors with NO DID~~ **re-superseded 2026-08-30 by DD 57081857: EVERY actor incl. Characters mints a DID; the DID IS the actor key (no internal UUID)** · ratings anchored to commission membership (2-dim Creator/Commissioner, 1–10★ positive-only) · fact-anchored deletion · payments = pre-alpha manual mark-as-paid · **nothing is derived from the changelog** — state is stored and read directly, the changelog is appended never replayed; the Eventlog is a frontend Surface, not an entity (DD 59310081) · **workflow board columns AND cards order by a fractional `Position` key (LexOrdering trait), never renumbered; placement is anchor-based (`after`), one card per board = MOVE; invariants in the DOMAIN, the store only backstops — NO reads-in-transaction / row locks / version columns for boards; commission elements keep integer `position`** (DD 62488577).
...
- `28246028` — Commission Surfaces — the Commission as a Tree of Typed Surfaces (SUPERSEDED IN PART 2026-08-04 → 45514754: the tree/inheritance dies; P/D/T modes, Total-default, server-side projection, envelope/payload survive)
- `28409880` — Commission Tree Storage — Adjacency Rows, Integer Position, Whole-Tree Load (SUPERSEDED IN PART 2026-08-04 → 45514754: adjacency/tree die; Postgres placement, envelope-columns/payload-jsonb, integer position, whole-load survive) · SUPERSEDED IN PART 2026-09-11 → 62488577: D3's "fractional-indexing upgrade" open row is TAKEN for workflow boards; D3 still governs commission content
...
- `29130754` — Commission Ownership Separation — View Grants & Account Placement (AMENDED 2026-09-04 inline: view grants are issued to **Users**, never Accounts; effective view = max(individual standing, the user's own key); membership confers no view; Total-key membership trade-off void; payload carries the grantee DID; D3/D4/D8-view/G4 amended, D1/D2/D6/D7 stand)
...
- `32178178` — The Eventlog — the Derived Timeline & Source Streams (SUPERSEDED IN PART 2026-09-04 → 59310081: the Eventlog is a live frontend Surface with NO storage — D2 no-live-hydration, D7 supersession, D8 eventlog-chain die; the 2026-07-27 causal model 'nothing computes from the changelog' survives and is ratified)
- `59310081` — The Changelog as the Only Record — Plugin Appends, Render-First Readability & the Eventlog as a Frontend Surface (DECIDED 2026-09-04; changelog = the ONLY durable record, append-only, atomic with state, NOTHING derived from it [state has one home, read cost history-independent]; plugins append via `changelog:write` scope as opaque `plugin_recorded` with mandatory sentence, rate-limited, security-review at build; human-readable = RENDERING contract [core kinds render at read from typed payloads, R11 holds; plugin entries store their sentence]; Eventlog = live frontend Surface composing paginated changelog + Portals, non-evidentiary; entries cursor-paginated + opaque cursor + R7 wrap; carve-outs: Golem read feed, ZMVP-114 verification, analytics, deadline-sweep idempotence fold; slot declare + invitations gain entries, maturity stays unrecorded; supersedes-in-part 32178178 + 30408741)
...
- `39944194` — Frontend Stack — Server-Only Effect & the Runes Seam (**DECIDED / GOVERNING — this is the current frontend decision.** The Dialect/ZesTTY (46596098) and the Rust/Leptos (47939586) explorations that followed it were both RETIRED 2026-08-09; the stack stands as specified here: SvelteKit + Svelte 5 + TypeScript + server-side Effect. DECIDED 2026-07-22; Effect only under `src/lib/server/**`, seam = `ManagedRuntime.runPromise` at hooks/loads/actions, runes never see a fiber; universal REJECTED [fibers-vs-runes two schedulers; no v1 client→PDS path per 26935298/29622283], revisit trigger = post-v1 direct PDS reads; RFC 9457 → tagged error union [23592962]; Effect Schema at untrusted boundaries [no Zod/Valibot] — **D4 AMENDED 2026-07-25 by 40992770: the API boundary decodes via the contract's GENERATED decoder (protobuf-es fromJson, server-side only); Effect Schema keeps every boundary the contract doesn't describe**; Layers = ports-by-role with in-mem test Layer [adapter-mem parity]; wraps @atproto/oauth-client-node, doesn't reimplement; no Option type — strict-null `T | undefined`; neverthrow considered & declined; scaffold ticket ZMVP-156)
- `42762241` — Surface Tree on the Wire — Projected Shape, oneof Discriminants & the Depth Cap (SUPERSEDED IN PART 2026-08-04 → 45514754: SurfaceTree/SurfaceNode, nesting, depth cap, oneof discriminant die — never minted; opaque_json payload, created_by-off, withheld-at-birth survive) (was DECIDED 2026-07-28; the commission SURFACE tree's first wire shape = ZMVP-75's projected shape minted NOW — messages `SurfaceTree`/`SurfaceNode` [never "CommissionTree", ambiguous vs the commission-to-commission hierarchy 29458433]; real nested children + write-side depth cap ≤63 [companion ZMVP-164; Rust decode dies at 64, TS ~100]; surface-vs-component = **oneof** [one-way door taken deliberately, unknown arm renders fail-closed]; payload = `oneof payload { string opaque_json = 1 }` [Struct disqualified by 2⁵³ corruption; typed arms additive later — R11's third case: mixed ownership over a deferred vocabulary]; `created_by` OFF v1; withheld envelope minted at birth [absence ≠ withheld, R4]; backend enablement ZMVP-163, frontend ZMVP-153; precedent for 75/74/72 read-side)
- `45514754` — Commission Composition — Surfaces as Extension Points, Flat Elements and Tabs (DECIDED 2026-08-04; supersedes-in-part 28246028/28409880/42762241 — the recursive Surface/Component tree is replaced by FLAT typed Elements contributed into declared Surfaces + Tabs, the extension-point pattern [VS Code/Eclipse]; core is the FIRST CONSUMER [no privileged path, Title = pinned contribution]; GLOBAL invariant skeleton [Workflow prefills content, never reshapes]; commission = the formal root [visibility + maturity are plain commission fields]; per-element/surface/tab P/D/T modes (same-day amendment), effective = min(tab, surface, element) — fixed arity, surface structure skeleton-fixed but surface modes per-commission data; containment ceiling stays free-by-construction (min clamps), only the grant ceiling is gated; Total default, server-side projection unchanged; address-by-id NEVER parent pointers [future `group` element = additive nesting by reference — reversible]; ranked rails [declarative Elements first-class, Tab iframe the exception]; ONE shared write gate [plugin modes ≤ grant ceiling + per-plugin-per-commission bound; security-review mandatory]; normalized plugin chrome + per-surface render declarations [D11]; storage = commission_tab + commission_element, commission_nodes DROPPED; wire = flat repeated elements, NO recursive message in the corpus; type catalog promoted to BLOCKER; "Slot" stays the Character position — never the extension point; ZMVP-164 → Won't Do [#170 closed, PG-16 pin salvaged as #171], 163 rebodied flat, 153/75/74/96/18/149 rescoped)
- `47939586` — The Rust Frontend — Leptos, Two Binaries and the Contract Leaf Crate (**REVERTED 2026-08-09 by the Engineer — withdrawn the day after it was decided; scope cost + use-case fit. The frontend stays plain SvelteKit + TypeScript (39944194 governs). Page kept as record; epic "The Rust Side" [ZMVP-184] closed Won't Do.** Was DECIDED 2026-08-08; frontend rewritten in Rust on Leptos 0.8.x EXACT-PINNED, retiring SvelteKit/Svelte/Effect/ZesTTY — supersedes 46596098 ENTIRELY + 39944194 by replacement; ONE LANGUAGE, TWO BINARIES [frontend = own leptos_axum SSR binary; 11763713 one-origin topology holds, BFF glue PORTS to tower middleware — /security-review mandatory on the Origin/CSRF layer]; contract leaf crate at `contract/rust/` OUTSIDE backend/ [wasm32-capable prost/pbjson, fourth instance of 40992770 D11; Cargo path-dep frontend→backend/crates/** BANNED]; no-JS forms doctrine survives via ActionForm-class PE; npm at the WRAP TIER [pre-bundled ESM + wasm-bindgen extern; web components = the valve]; Option ban VOIDED [C1], "client bundle untouched" dies as bundle-rule [C4], kind-discriminant + case→kind adapters DISSOLVE [prost oneof = Rust enum], Result seam goes native; Dioxus = NAMED FALLBACK w/ 3 checkable reopen conditions [#1996 PE forms / #4631 wasm_split under SSR / funding resolves]; rulebook 37519361 binds the frontend; migration TOTAL: scaffold → leaf crate → BFF/auth port [Opus, sec-review] → screens → specs → frontend/web deletion terminus; epic "The Rust Side"; spike annex owed [Dioxus half measured: ~386 KB gzip, 2-entry-point PE forms, silent wasm-opt fallback]; basis = Sacrifice Ledger + Dioxus-pass workflows + twin spikes)
- `46596098` — The Dialect — ZesTTY Adoption: Committed Twins, the Result Seam and the Tag Pin (**RETIRED 2026-08-09 — ZesTTY was NEVER adopted into Zurfur; the frontend stays plain SvelteKit + TypeScript (39944194 governs). ZesTTY continues as the Engineer's standalone project, decoupled from Zurfur. NOTE: never reached `main` — no cleanup owed. Not superseded-by-Leptos [that decision was itself reverted].** Was DECIDED 2026-08-05; ZesTTY [github.com/ZurNetwork/ZesTTY, the Engineer's Rust-flavored TS superset] = the frontend AUTHORING language; mode = COMMITTED TWINS [.zts authored, generated .ts committed @generated, `just zts-gen`, zts-check drift tripwire in CI — third instance of 40992770 D11; twins ARE the credible exit]; seam law = runApi returns Result [single Effect→Result point; Effect keeps Layers/Schema/orchestration server-side; seam may migrate server-ward, trigger = `?` shipping]; `kind` = client-facing discriminant standard [four-discriminant cleanup; protobuf case→kind via hand-written tested adapters]; coupling = npm EXACT pins @zestty/check@0.1.0 + @zestty/native@0.1.0 [same-day amendment post-publish; binary in-tarball, no postinstall; dual tag-pinned checkouts = documented fallback]; containment via twins [eslint parses them]; Option pre-bind extends 39944194 D7; migration TOTAL + Engineer-driven, epic "The Dialect", sequenced after ZMVP-148; OPEN: Ok/Err-vs-domain naming, lang="zts" reopen [upstream #15], direct-import reopen [#17], .zts formatting, rulebook scope; amends 39944194 + 23592962 + 40992770 D5/D11)
- `55836674` — The Application Layer — Use Cases, DTOs and Ports (DECIDED 2026-08-27, recorded from the ZMVP-205 rulings; one plain `pub async fn` per use case, no mediator; `XQuery`/`XCommand` in → flat `XResult` of domain VALUES never ENTITIES out, both directions; per-module `Ports` struct of `&dyn`; `now` + runtime config as parameters; the use case owns the transaction [`application::transaction`, no driver/background job opens one — sweep moves down; terminus = `pub(crate)`]; authn = driver / authz + orchestration = application / invariants = domain; one terse-`Display` error enum per module; `shared` = the one leaf crate domain may consume, consumers-only deps; parity test per two-driver use case; amends 11763713 [composition root], 24150017 D1, 23592962 C1; follow-ups ZMVP-208/209)
- `57081857` — Actor Addressing — DID as the Only Identifier, Everywhere (DECIDED 2026-08-30; did:plc is the ONLY actor identifier on wire/domain/store — `UserId(Did)`/`AccountId(Did)`/`CharacterId(Did)`, actor tables keyed by `did` NOT NULL, surrogate UUID dropped, no resolver seam; EVERY actor mints a DID at creation, Characters included [handle-less; Character handles = roadmap] — supersedes-in-part 34013187 + 54427650, restores 4882433 Option 2; non-actors keep opaque ids; handle = alias never key; tombstone Bluesky-style [mark, never NULL]; plugins receive DIDs, scope bounds [amends 24543244 D3; security-review DONE 2026-08-30 — no blocking threat, two findings recorded on 24543244: F1 by-DID membership oracle must return uniform not-found + rate-limit, F2 revocation no longer un-identifies]; contract break by BASELINE RESET [amends 40992770 R9]; amends 11763713 Addressing + 55836674 D3/D7 + 26247170)
- `54427650` — Character public presence — DID + full PDS repo for public Characters, Index-only for private (DECIDED 2026-08-23; supersedes 26935298's Character scope; source of record = vulpes docs/characters-atproto.md) — index entry added 2026-08-27, page details not re-fetched
...
- `62488577` — Board Ordering — Fractional Position Keys, Anchor Placement and One Card per Board (DECIDED 2026-09-11; workflow columns + cards order by a fractional `Position` minted by the `LexOrdering` trait — deterministic jitter = low 18 bits of the element's UUIDv7 `rand_b` → 3 base-62 chars, last ≠ `0`; `midpoint` never stops at a prefix of `hi`; `(position, id)` TOTAL order, ties legal, insert between tied cards lands after the tie group; placement = `after: Option<CommissionId>` resolved in the domain, stale anchor → 409 `anchor-not-in-column`, `index` RETIRED before ever minted [no contract break; `/placements` minted last]; one card per board = MOVE as one UPDATE preserving alias/archived/plugin metadata, schema BACKSTOPS only [`workflow_id` on the edge, PK `(workflow_id, commission_id)` NOT DEFERRABLE = the ON CONFLICT arbiter, composite FK, deferrable unique dropped]; invariants in the DOMAIN — REJECTED by name: read-in-txn, `FOR UPDATE`, version columns, adapter retries, counter SQL, DEFERRABLE tricks, `MERGE`; four single-fact ports + one ordered board read; soft column cap [+1 accepted] → `shared::settings`; scope fence: commission elements keep integer position [45514754 D8], 28409880 D3 open row TAKEN; LOCKED LISTS OUT OF SCOPE [Engineer 2026-09-11]; "Column" canonical, "List" UI label; companions on #197: `owning_account_of → Option`, `IndexOutOfRange` → superseded by `AnchorNotInColumn`, `account_id` dropped from rename/reposition; prior art Trello `pos` / Figma fractional indexing / Wallace 2022 jitter / RFC 9562 / PG ON CONFLICT)
