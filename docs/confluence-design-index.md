...
### Design decisions (DDs) — 2026-08 wave
- `54427650` — Characters on ATProto — Public DIDs, PDS Accounts & ACP Ownership (DECIDED 2026-08-11 in vulpes, reshaped 2026-08-22, synced into DESIGN 2026-08-23; **source of record = vulpes `~/code/vulpes/docs/characters-atproto.md`** + `ccs.md`/`acp.md`; post-alpha roadmap — v1 scope unchanged. Public Character = own did:plc + full migratable PDS repo [kind `app.zurfur.acp.identity.character`]; private Character = Index-only, ZERO ATProto footprint; default public; publicize = consent-gated one-way door, de-publicize = teardown not rollback; ownership atproto-native via ACP = owner's `net.got-paws.acp.relationship.ownership` claim + character's attestation + rotation-key seniority — **Keeper = ACP owner** [Engineer 2026-08-23]; transfer = key rotation + claim swap, **72h-final**; co-ownership = priority-ordered keys; hard requirements = senior owner key + CAR export, layout D w/ client-side user key [F46]; supersedes-in-direction 34013187's "no DID" amendment [schema survives — publicness determines DID] + 26935298's Character minting [B1b decided for Characters: own repo]; OPEN: custody layout D vs 26804226 [flag-only], Character handles)
