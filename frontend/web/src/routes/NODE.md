...
fs:
  ...
  - name: login/
    ...
    node: false
  - name: mock/
    role: dev-only mock signin (routes/mock/signin/+server.ts) — completes the login redirect loop with no OAuth/PDS/backend; fails closed with a 404 unless mock mode is enabled
    node: false
  ...
  - name: +page.svelte
    ...
    node: false
  - name: layout.server.spec.ts / page.svelte.spec.ts
    role: specs for the root layout load and the landing page (the `+` is dropped in route spec names)
    node: false
---
...
**Entry points:** `+layout.server.ts` (the once-per-render whoami) · `login/+page.server.ts`.

**Refs:** none of its own — the `/` landing-page ruling is recorded one level up in `frontend/web/src/NODE.md`.
